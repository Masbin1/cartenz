...
export class CommandRunner {
  ...
  constructor(@Inject(APP_CONFIG) config: AppConfig) {
    ...
    this.pullScript = config.provisioning?.enabled
      ...
      : '';
    // ADR-052. Same posture as the pull script: empty is the off switch, and it
...
/**
 * A preview reference the root-run preview script will accept (ADR-052).
 *
 * It names a directory, a systemd unit and part of a database name, so it is a
 * plain lowercase alphanumeric token and nothing else. Generated by the service
 * as 16 hex characters; re-validated here independently of that.
 */
 ...
 *   domain.
 *
 *   `sudo -n <pull-script> <project-name> <repository-url> <branch>` — the
 *   project-pull script (ADR-049), with the credential on stdin.
 *
 *   `sudo -n <preview-script> <start|stop> <project-name> <ref>` — the ephemeral
...
export function assertProvisioningInvocation(
  ...
  // The preview script is the one shape whose second argument is a subcommand
  // rather than a project name, so it is handled before the shared project-name
  // check below. ADR-052: exactly `start <project-name> <ref>` or
  // `stop <project-name> <ref>`; the job file and patch live in a fixed staging
  // directory the script reads, never a path from this argument vector.
  if (isPreview) {
    const subcommand = args[2];
    if (subcommand !== 'start' && subcommand !== 'stop') {
      throw new CommandArgumentError(
        `The preview script takes "start" or "stop" as its subcommand; got "${String(subcommand)}".`,
      );
    }

    const previewName = args[3];
    if (!previewName || !PROVISIONING_PROJECT_NAME.test(previewName)) {
      throw new CommandArgumentError(
        'sudo preview requires a valid project name as the third argument.',
      );
    }

    const ref = args[4];
    if (!ref || !PREVIEW_REF.test(ref)) {
      throw new CommandArgumentError(
        'sudo preview requires a 16-character lowercase alphanumeric preview reference as ' +
          `the fourth argument; got "${String(ref)}".`,
      );
    }

    if (args.length !== 5) {
      throw new CommandArgumentError(
        `The preview script takes exactly "-n <script> <start|stop> <project-name> <ref>"; ` +
