...
export class ProjectProvisioningService {
  ...
  async provision(input: ProvisionProjectInput): Promise<ProvisionProjectResult> {
    ...
    try {
      ...
      const url = this.config.provisioning.baseDomain
        ? `http://${input.technicalName}.${this.config.provisioning.baseDomain}`
        : `http://127.0.0.1:${port}`;

      this.logger.log(`Provisioned "${input.technicalName}" at ${url} (port ${port})`);

      return { provisioned: true, port, url, error: null, stdout: result.stdout, stderr: result.stderr };
    } catch (error) {
      ...
      return { provisioned: false, port: null, url: null, error: message, stdout: '', stderr: '' };
    }
  }
