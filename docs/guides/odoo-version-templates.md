...
> needs `google-auth`: install the pinned set from §7 — `pip install google-auth`
> alone upgrades `cryptography` and breaks every module install on this
> Python 3.10 venv — then rerun with `only=full`). **Not yet checked:** whether existing projects
...
| `pip install google-auth` (unpinned) then **every** module install dies with `ModuleNotFoundError: No module named 'cryptography.hazmat.backends.openssl.x509'`, or `AttributeError: module 'lib' has no attribute 'GEN_EMAIL'`, while loading `base` | `google-auth` declares `cryptography>=38.0.3` and pip is happy to upgrade `cryptography` to satisfy it. This venv is Python 3.10, where Odoo pins `cryptography==3.4.8` + `pyopenssl==21.0.0` + `urllib3==1.26.5`; `urllib3` 1.26 imports the `cryptography.hazmat.backends.openssl.x509` module that cryptography ≥42 removed, and `pyopenssl` 21 predates the `lib.GEN_EMAIL` move. Nothing to do with `cloud_storage_google` — `base` itself can no longer load, so **every** provisioning fails | Never install `google-auth` unpinned here. Install the tested set that satisfies both directions: `pip install "cryptography==3.4.8" "pyopenssl==21.0.0" "urllib3==1.26.5" "google-auth==2.40.3"`. Verify with the import chain in §5 before provisioning again. Do **not** take the `cryptography==42.0.8` / `pyopenssl==24.1.0` pair from Odoo's `requirements.txt` — those are the `python_version >= '3.12'` pins |
...
| Every app icon is blank in `/odoo` (app switcher) on a provisioned project | The template's filestore was never kept. `ir.ui.menu.web_icon_data` is computed once when a module is installed and stored as an attachment in `data_dir/filestore/<db>` — not re-read from the addon later. A build that installs into a scratch `data_dir` that is then deleted loses every icon, and `CREATE DATABASE … TEMPLATE` never carries a filestore anyway | Fixed in both scripts: `build-odoo-templates.sh` now stores each build's filestore under `/opt/odoo/templates/filestore/<template>` (the layout `create-project-db.sh` already reads), and the selective install writes straight into the project's own `data_dir`. **Existing templates must be rebuilt**: `build-odoo-templates.sh 19.0 … base` (and `full` once `google-auth` is in the venv). Projects already created this way cannot be repaired by "Update Apps List" — reinstall their modules or recreate the project |
